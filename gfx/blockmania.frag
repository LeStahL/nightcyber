...
void illuminate(in vec3 x,in vec3 n,in vec3 dir,in vec3 l,inout vec3 col,in vec2 s)
{
    ...
    {
        col=.1*mix(vec3(.18,.16,.20),vec3(.38,.36,.39),clamp(iTime-21.651+.25,0.,1.)/.5);
        col=.7*col
        +.8*col*max(dot(l-x,n),0.)
        +.3*col*pow(max(dot(reflect(l-x,n),dir),0.),1.);
    }
    ...
    {
        col=mix(vec3(.52,.24,.24),vec3(.65,.35,.36),clamp(iTime-21.651+.25,0.,1.)/.5);
        col=.7*col
        +.8*col*max(dot(l-x,n),0.)
        +.3*col*pow(max(dot(reflect(l-x,n),dir),0.),1.);
    }
    ...
    {
        col=.2*mix(vec3(.08,.65,.86),vec3(.96,.41,.35),clamp(iTime-21.651+.25,0.,1.)/.5);
        col=.7*col
        +.8*col*max(dot(l-x,n),0.)
    ...
    {
        col=.2*mix(vec3(.62,.60,.60),vec3(.93,.68,.30),clamp(iTime-21.651+.25,0.,1.)/.5);
        col=.7*col
        +.8*col*max(dot(l-x,n),0.)
        +1.3*col*pow(max(dot(reflect(l-x,n),dir),0.),1.);
    }
    ...
    {
        col=.4*mix(vec3(.82,.12,.21),.1*c.xxx,clamp(iTime-21.651+.25,0.,1.)/.5);
        col=.7*col
        +.8*col*max(dot(l-x,n),0.)
        +1.3*col*pow(max(dot(reflect(l-x,n),dir),0.),1.);
    }
...
void image210(inout vec3 col,in vec2 uv)
{
    ...
    dbox210(vec3((uv-.5*size*c.xy).yx,.15),size,sdf);
    d=sdf.x;
...
{
    vec2 da=abs(x)-b;
    d=length(max(da,c.yy))+min(max(da.x,da.y),0.);
}
...
col=mix(col,mix(col,vec3(.87,.24,.59),.5),sm(max(fa,-ff/8.)));
...
// if(d < 1.e2)
{
    ...
    // for(d=1.e-2; x.z<.5; )
    {
        ...
        if(x.z>0.)break;
        float y=s.x*s.x/(2.*ph)/12.;
        float da=sqrt(s.x*s.x-y*y);
        res=min(res,100.*da/max(0.,d-y));
        ph=s.x;
        d+=min(s.x,5.e-4);
    }
...
if(isa)
{
    ...
    col=mix(col,mix(col,vec3(.35,.47,.63),.3),sm(abs(nar)-.004));
...
col=mix(col,mix(vec3(.67,.13,.18),vec3(.06,.36,.38),abs(y.x)/.5),smoothstep(-.5,.01,y.y));
