...
void* malloc(size_t size) { return GlobalAlloc(GMEM_ZEROINIT, size); }
...
{
  static unsigned char header[54] = {0x42, 0x4D, 0x36, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x36, 0x00, 0x00, 0x00,
                                     0x28, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00,
                                     0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0xC4, 0x0E, 0x00, 0x00,
...
int WINAPI demo(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow)
{
  ...
  RegisterClass(&wca);
  HWND lwnd = CreateWindowEx(0,                   // Optional window styles.
  ...
  // Add "Resolution: " text
  HWND hResolutionText =
      CreateWindow(WC_STATIC, "Resolution: ", WS_VISIBLE | WS_CHILD | SS_LEFT, 10, 15, 100, 100, lwnd, NULL, hInstance, NULL);

  // Add resolution Combo box
  HWND hResolutionComboBox = CreateWindow(WC_COMBOBOX,
                                          TEXT(""),
                                          CBS_DROPDOWN | CBS_HASSTRINGS | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE,
                                          100,
                                          10,
                                          175,
                                          nuniqueresolutions * 15,
                                          lwnd,
                                          (HMENU)5,
                                          hInstance,
                                          NULL);
  ...
  // Add mute checkbox
  HWND hMuteCheckbox =
      CreateWindow(WC_BUTTON, TEXT("Mute"), WS_VISIBLE | WS_CHILD | BS_CHECKBOX, 10, 40, 100, 20, lwnd, (HMENU)6, hInstance, NULL);

  // Add "Antialiasing: " text
  HWND hAntialiasingText =
      CreateWindow(WC_STATIC, "FSAA: ", WS_VISIBLE | WS_CHILD | SS_LEFT, 10, 65, 100, 100, lwnd, NULL, hInstance, NULL);

  // Add Fullscreen Antialiasing combo box
  HWND hFSAAComboBox = CreateWindow(WC_COMBOBOX,
                                    TEXT(""),
                                    CBS_DROPDOWN | CBS_HASSTRINGS | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE,
                                    100,
                                    60,
  ...
  HWND hTXAAComboBox = CreateWindow(WC_COMBOBOX,
                                    TEXT(""),
                                    CBS_DROPDOWN | CBS_HASSTRINGS | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE,
                                    100,
                                    90,
  ...
  // Add scene selector
  HWND hSceneComboBox = CreateWindow(WC_COMBOBOX,
                                     TEXT(""),
                                     CBS_DROPDOWN | CBS_HASSTRINGS | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE,
                                     100,
                                     120,
  ...
  HWND hwndButton = CreateWindow(
      WC_BUTTON, "Offend!", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON, 185, 225, 90, 90, lwnd, (HMENU)7, hInstance, NULL);

#ifdef RECORD
  // Add record checkbox
  HWND hRecordCheckbox =
      CreateWindow(WC_BUTTON, TEXT("Record"), WS_VISIBLE | WS_CHILD | BS_CHECKBOX, 10, 150, 89, 20, lwnd, (HMENU)11, hInstance, NULL);
  ...
  // Add FPS selector
  hFPSDropdown = CreateWindow(WC_COMBOBOX,
                              TEXT(""),
                              CBS_DROPDOWN | CBS_HASSTRINGS | WS_CHILD | WS_OVERLAPPED | WS_VISIBLE,
                              100,
                              180,
  ...
  if (recording) {
    ...
    if (!CreateDirectory(record_filename, NULL)) {
      ...
      if (err == ERROR_ALREADY_EXISTS)
        ...
        printf("Parts of the path do not exist.\n");
    }
  }
#endif
  ...
  GLint attribs[] = {WGL_CONTEXT_MAJOR_VERSION_ARB,
                     opengl_settings.majorVersion,
                     WGL_CONTEXT_MINOR_VERSION_ARB,
                     opengl_settings.minorVersion,
                     WGL_CONTEXT_PROFILE_MASK_ARB,
                     opengl_settings.profile == CompatibilityProfile ? WGL_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB
...
void set_sound_playback_time(double time_begin) { set_sound_playback_range(time_begin, t_end); }
