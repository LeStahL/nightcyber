...
RenderViewWidget::RenderViewWidget(QWidget* parent) : QOpenGLWidget(parent) { setup(); }

void RenderViewWidget::initializeGL() { DemoApi::api.initializeGL(); }

void RenderViewWidget::resizeGL(int w, int h) { DemoApi::api.resizeView(w, h); }

void RenderViewWidget::paintGL() { DemoApi::api.renderFrame(); }
